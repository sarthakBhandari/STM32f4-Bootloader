...
#define BAUD_RATE   (115200) // actual bytes/second ~ 11520 (there are some other surrounding bits)
...
void usart2_isr(void){
    ...
    const bool recieved_data = usart_get_flag(USART2, USART_FLAG_RXNE);
...
uint32_t uart_read(uint8_t* data, const uint32_t length){
    ...
    for (uint8_t bytes_read = 0; bytes_read < length; bytes_read ++){
